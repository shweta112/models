...
def recursive_parse_txt_to_dict(txt_lines, example):
    ...
    result = {}
    result['filename'] = example.replace('_', '/').split('.')[0]
    if len(txt_lines) == 0:
        return result
    result['object'] = []
    for line in txt_lines:
        ...
        bndbox['xmin'] = left
        bndbox['ymin'] = top
        bndbox['xmax'] = int(left) + int(w)
        bndbox['ymax'] = int(top) + int(h)
...
def dict_to_tf_example(data, image_dir, add_image_dir, label_map_dict,
  ...
  """
  set, v, img = data['filename'].split('/')

  full_path = os.path.join(image_dir, set, 'Visible_' + v, img + '.png')
  add_path = os.path.join(add_image_dir, set, 'LWIR_' + v, img + '.png')
